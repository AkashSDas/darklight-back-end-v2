...
export const verifyAuth: AsyncMiddleware = async (req, res, next) => {
  ...
  // if user is logged in using Social Auth then `req.user` will be set
  if (req.user) return next();

  // If the user is logged in using the basic email and password auth then
  // checking for the `req.headers.authorization` header
